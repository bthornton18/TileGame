/*
*** FILE NAME   : TileGame.cpp
*** DESCRIPTION : This program will run an A* algorithm to solve an 8 tile puzzle
*** DATE        : Sep. 2017
*** WRITTEN By  : Ben Thornton
*/
...
class SearchTree {
public:
	SearchTree::SearchTree(Tile[3][3], int);
	Tile CurrentState[3][3];
	int Depth;
	int Weight;
	SearchTree* up;
	SearchTree* down;
	...
	//------------------------------------------------------------------------------------
	void SetWeightH2() {
		Weight = NumTilesOutOfPlace();
	}
	//------------------------------------------------------------------------------------
	void GenerateChild(string ChildToGen, int BlankRow, int BlankColumn) {
		Tile temp;
		Tile NextState[3][3];
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
		...
		if ("Left") {
			temp = NextState[BlankRow][BlankColumn];
			NextState[BlankRow][BlankColumn] = NextState[BlankRow][BlankColumn - 1];
			NextState[BlankRow][BlankColumn - 1] = temp;
		}
		else if ("Right") {
			temp = NextState[BlankRow][BlankColumn];
			NextState[BlankRow][BlankColumn] = NextState[BlankRow][BlankColumn + 1];
			...
			temp = NextState[BlankRow][BlankColumn];
			NextState[BlankRow][BlankColumn] = NextState[BlankRow + 1][BlankColumn];
			NextState[BlankRow + 1][BlankColumn] = temp;
		}
		else if ("Down") {
			temp = NextState[BlankRow][BlankColumn];
			NextState[BlankRow][BlankColumn] = NextState[BlankRow - 1][BlankColumn];
	...
	int DistanceToGoal() {
		...
		{
			for (int j = 0; j < 3; j++) {
				TotalDistance += SingleTileDistance(i, j, this->CurrentState[i][j].label);
			}
		}
...
SearchTree::SearchTree(Tile State[3][3], int level) {
	this->Depth = level;
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++) {
			this->CurrentState[i][j] = State[i][j];
			cout << State[i][j].label << endl;
		}
	}
}
